...
- 拦截分类 AJAX，使用固定周榜/月榜响应，不消耗 NYT API 配额
- 缺少更新时间、频率和榜单日期的旧 localStorage 缓存应失效并重新请求
- 月榜 AJAX 切换后显示该分类自己的更新时间、榜单日期和中文提示
- 切到英文后显示英文月榜提示，且不重新请求分类 API
- 周榜隐藏月榜提示；回切月榜命中缓存并恢复月榜自己的元数据

**期望输出：**
```
[PASS] 初始访问 0 个分类 API 请求
[PASS] 旧缓存失效，中文月榜提示和更新时间来自 AJAX 响应
[PASS] 英文月榜提示正确，切语言 0 个分类 API 请求
[PASS] 周榜隐藏月榜提示并显示自己的更新时间
[PASS] 回切月榜命中缓存并恢复自己的时间、榜单日期和中文提示
```

默认访问 `http://127.0.0.1:8000`；需要测试其他端口时设置
`BOOKRANK_BASE_URL`。
