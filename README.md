...
- **`CODE_WIKI.md`**：单文件完整技术文档（适合在线浏览和搜索）
- **`Code Wiki/` 目录**（v0.9.80+）：拆分到 Obsidian vault 的知识库，按章节组织
  - 18 个章节笔记 + 1 个索引页，支持 wikilink 双向链接导航
  - 适合在 Obsidian 中本地查阅、做笔记、建立关联
  - 目录：`BookRank3/Code Wiki/`
...
1. 在 Render 平台创建新的 Web Service，连接 GitHub 仓库。
2. 使用仓库根目录 `render.yaml` 作为 Blueprint，或手动配置：
   - Name: bookrank
   - Region: Oregon / Singapore（根据实际网络情况选择）
   - Branch: main
   - Build Command: `pip install -r requirements-prod.txt`
   - Start Command: `gunicorn -c gunicorn.conf.py run:application`
   - Health Check Path: `/health/ready`
3. 在 Render Dashboard 添加环境变量（参考 `.env.example`）：
...
- v0.9.79 - 综合审计整改收尾（2026-07-05）：完成代码质量、测试覆盖、安全、性能、配置环境、部署回滚、监控告警、数据库、文档与用户体验 10 维度审计；路由层 `db.session` 下沉 Service、修复 2 处 N+1 查询、`admin.py` 补齐 CSRF、接入 Sentry/任务失败告警、`render.yaml` 关闭自动部署并改为 `/health/ready` 健康检查、新增 10 份审计报告与 3 份 Runbook；README / CHANGELOG / VERSION / onboarding / API 文档同步更新；`make check` 与 `pytest` 全量通过，覆盖率 ≥73%
- v0.9.78 - 移动端详情页 Tab 化 + Tab 改名 + 语言切换：书籍详情和获奖详情加"图书简介 / 详细信息"两个 Tab（切到"详细信息"时懒加载 Google Books 介绍）；首页顶部 nav 删除搜索放大镜；底部 4 个 Tab 改为 首页 / 获奖书单 / 出版社 / 我的；详情页底部"返回榜单"按钮去掉；顶部右上角加地球图标，下拉切换"简体中文 / English"，复用 `book-i18n.js`，持久化到 `localStorage.bookrank_language`；2136 passed，移动端测试从 20 个增至 30 个
