...
#### 1. 删除类型检查缓存
- **删除**：`.mypy_cache/` 目录（约 17MB）
- **说明**：mypy 类型检查的缓存文件，可安全删除，下次运行类型检查时会自动重建

#### 2. 删除应用缓存
- **删除**：`cache/` 目录
- **说明**：应用运行时的缓存文件，删除后会自动重新生成

#### 3. 删除临时数据文件
- **删除**：`instance/test.db` - 测试数据库
- **删除**：`bestsellers.db` - 旧版本数据库文件
- **删除**：`translation_cache.json` - 翻译缓存文件
- **删除**：`Book_Info_Translated.xlsx` - 临时 Excel 文件

### 影响
- 释放约 18MB 磁盘空间
- 不影响应用正常运行，所有删除的文件都可自动重建
- 下次运行 `mypy` 时会重新生成类型缓存，可能需要稍长时间
...
### 热修复
- **Jinja2 语法错误导致 500 崩溃**: `clean_isbn|is digit` 不是合法 Jinja2 语法，改用字符替换法验证纯数字（`replace('0-9','')` 后检查是否为空）
