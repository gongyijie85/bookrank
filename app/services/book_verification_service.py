...
class BookVerificationService:
    ...
    def verify_book(self, book: AwardBook) -> tuple[bool, dict]:
        ...
        result = {'book_id': book.id, 'title': book.title, 'isbn13': book.isbn13, 'checks': {}, 'passed': False}
        ...
        all_critical_passed = all(
            result['checks'].get(check, {}).get('passed', False)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if api_title and book.title:
                    ...
                    if book.title.lower() in api_title.lower() or api_title.lower() in book.title.lower():
                        check['details'].append('✅ 书名匹配')
                    else:
                        self.warnings.append(f'书名可能不匹配: 本地"{book.title}" vs API"{api_title}"')
                        check['details'].append('⚠️ 书名差异')
    ...
    def verify_all_pending(self, limit: int = 10) -> list[dict]:
        ...
        # 查询待验证或验证失败的图书
        pending_books = (
            AwardBook.query.filter(AwardBook.verification_status.in_(['pending', 'failed'])).limit(limit).all()
        )
        ...
        for book in pending_books:
            try:
                ...
                book.verification_checked_at = datetime.now(UTC)
                book.verification_errors = json.dumps(
                    {'errors': self.errors, 'warnings': self.warnings, 'checks': result['checks']}
                )
