...
class RecommendationService:
    ...
    def get_personalized_recommendations(self, session_id: str, limit: int = 10) -> dict[str, Any]:
        """
        获取个性化推荐
    ...
    def _analyze_user_interests(self, session_id: str, viewed_books: list[UserViewedBook]) -> dict[str, Any]:
        ...
        metadata_books = BookMetadata.query.filter(BookMetadata.isbn.in_(isbns)).all() if isbns else []

        award_books = AwardBook.query.filter(AwardBook.isbn13.in_(isbns)).all() if isbns else []
        ...
        # 统计最常见的关键词和作者
        interests['keywords'] = [k for k, v in Counter(interests['keywords']).most_common(5)]
        interests['authors'] = [a for a, v in Counter(interests['authors']).most_common(3)]
    ...
    def _extract_keywords(self, text: str) -> list[str]:
        ...
        # 过滤停用词（覆盖常见英语高频词，减少推荐噪音）
        stop_words = {
            'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to',
            'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be',
            'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did',
            'will', 'would', 'shall', 'should', 'may', 'might', 'can',
            'not', 'no', 'nor', 'so', 'if', 'then', 'than', 'that',
            'this', 'these', 'those', 'it', 'its', 'he', 'she', 'they',
            'them', 'his', 'her', 'their', 'my', 'your', 'our', 'we',
            'from', 'into', 'about', 'between', 'through', 'after',
            'before', 'above', 'below', 'up', 'down', 'out', 'off',
            'over', 'under', 'again', 'further', 'once', 'here', 'there',
            'when', 'where', 'why', 'how', 'all', 'each', 'every',
            'both', 'few', 'more', 'most', 'other', 'some', 'such',
            'only', 'own', 'same', 'also', 'just', 'now',
        }
        words = [w for w in words if w not in stop_words and len(w) > 2]

        return words

    def _recommend_by_interests(self, interests: dict[str, Any], limit: int) -> list[dict]:
        """
        基于用户兴趣推荐图书
    ...
    def _generate_recommendation_reason(self, interests: dict[str, Any]) -> str:
        ...
        if interests.get('categories'):
            cat_names = [self._categories.get(c, c) for c in interests['categories'][:2]]
            if cat_names:
    ...
    def _get_popular_recommendations(self, limit: int) -> dict[str, Any]:
        ...
        # 获取最近几年的热门获奖图书
        books = (
            AwardBook.query.filter_by(is_displayable=True)
        ...
        return {'recommendations': recommendations, 'reason': '热门获奖图书推荐', 'based_on': 'popular'}
    ...
    def _recommend_similar_books(self, target_book: AwardBook, limit: int) -> dict[str, Any]:
        ...
        return {
            ...
            'based_on': 'similarity',
            'reference_book': {'id': target_book.id, 'title': target_book.title, 'author': target_book.author},
        }

    def _recommend_by_award(self, award_id: int, category: str | None, limit: int) -> dict[str, Any]:
        """基于特定奖项推荐"""
        query = AwardBook.query.filter_by(award_id=award_id, is_displayable=True)
        ...
        return {'recommendations': recommendations, 'reason': '同奖项其他年份获奖图书', 'based_on': 'award_similarity'}

    def _recommend_by_category(self, category: str, limit: int) -> dict[str, Any]:
        """基于分类推荐"""
        books = (
            AwardBook.query.filter_by(category=category, is_displayable=True)
            .order_by(AwardBook.year.desc(), AwardBook.rank.asc())
            .limit(limit)
            .all()
        )
        ...
        return {'recommendations': recommendations, 'reason': f'{category}分类的获奖图书', 'based_on': 'category'}

    # ==================== 智能推荐（综合） ====================

    def get_smart_recommendations(self, session_id: str | None = None, limit: int = 10) -> dict[str, Any]:
        """
        获取智能推荐（综合多种策略）
        ...
        # 尝试个性化推荐
        if session_id:
            personalized = self.get_personalized_recommendations(session_id, limit)
