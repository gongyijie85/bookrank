...
from .api_utils import api_retry, create_session_with_retry
...
class WikidataClient:
    ...
    def __init__(self, timeout: int = 60):
        ...
        self._session.headers.update(
            {'User-Agent': 'BookRank/2.0 (bookrank@example.com)', 'Accept': 'application/sparql-results+json'}
        )
    ...
    def query_award_winners(
        ...
        try:
            response = self._session.get(
                self._base_url, params={'query': sparql_query, 'format': 'json'}, timeout=self._timeout
            )
