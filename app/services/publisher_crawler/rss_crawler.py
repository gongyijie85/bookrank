...
class PublisherRSSCrawler(BaseCrawler):
    ...
    def _parse_rss_item(self, elem, ns: dict) -> dict[str, Any] | None:
        ...
        return {
            ...
            'description': self._get_text(elem, 'description') or self._get_text(elem, 'content:encoded', ns) or '',
            'pub_date': self._get_text(elem, 'pubDate') or self._get_text(elem, 'dc:date', ns) or '',
            'author': self._get_text(elem, 'dc:creator', ns) or self._get_text(elem, 'author') or '',
    ...
    def _parse_atom_entry(self, elem, ns: dict) -> dict[str, Any] | None:
        ...
        content = (
            self._get_text(elem, 'atom:content', ns)
            or self._get_text(elem, 'atom:summary', ns)
            or self._get_text(elem, 'content')
            or self._get_text(elem, 'summary')
            or ''
        )

        pub_date = (
            self._get_text(elem, 'atom:published', ns)
            or self._get_text(elem, 'atom:updated', ns)
            or self._get_text(elem, 'published')
            or self._get_text(elem, 'updated')
            or ''
        )
        ...
        if author_elem is not None:
            author = self._get_text(author_elem, 'atom:name', ns) or self._get_text(author_elem, 'name') or ''
