...
class MacmillanCrawler(BaseCrawler):
    """
    Macmillan 出版社爬虫
    ...
    def _check_crawl4ai(self) -> bool:
        ...
        try:
            import crawl4ai
            logger.info(f"✅ Macmillan: Crawl4AI 可用")
            return True
        except ImportError:
            logger.info(f"ℹ️ Macmillan: Crawl4AI 未安装，仅使用传统 requests")
            return False
    ...
    async def _crawl_with_crawl4ai_async(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"🕸️ Macmillan: 使用 Crawl4AI 爬取: {url}")
            ...
            async with AsyncWebCrawler(config=browser_config) as crawler:
                ...
                if result and result.success and result.html:
                    logger.info(f"✅ Macmillan: Crawl4AI 爬取成功")
                    return result.html
            return None
        except Exception as e:
            logger.warning(f"⚠️ Macmillan: Crawl4AI 出错: {e}")
            return None
    ...
    def _crawl_with_crawl4ai(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"⚠️ Macmillan: Crawl4AI 同步调用失败: {e}")
            return None
    ...
    def _make_request_with_fallback(self, url: str) -> tuple[Optional[BeautifulSoup], Optional[str]]:
        ...
        """
        logger.info(f"🔄 Macmillan: 尝试传统 requests: {url}")
        response = self._make_request(url)

        if response:
            logger.info(f"✅ Macmillan: 传统 requests 成功")
            return self._parse_html(response.text), 'requests'

        if self._crawl4ai_available:
            logger.info(f"🔄 Macmillan: 降级到 Crawl4AI")
            html = self._crawl_with_crawl4ai(url)

            if html:
                logger.info(f"✅ Macmillan: Crawl4AI 成功")
                return BeautifulSoup(html, 'html.parser'), 'crawl4ai'

        logger.warning(f"❌ Macmillan: 所有方法都失败: {url}")
        return None, None
