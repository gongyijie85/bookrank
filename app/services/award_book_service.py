...
class AwardBookService:
    ...
    def _process_single_book(self, award: Award, book_data: dict, category: str) -> str:
        ...
        book = AwardBook(
            ...
            or book_details.get('author')
            or google_books_data.get('author')
            or 'Unknown',
    ...
    def _get_cover_url(self, isbn: str) -> str | None:
    ...
    def get_refresh_status(self) -> dict[str, Any]:
        ...
        if not last_refresh:
            return {'last_refresh': None, 'next_refresh': None, 'days_since_last': None, 'needs_refresh': True}
        ...
        try:
            ...
            return {'last_refresh': last_refresh, 'next_refresh': None, 'days_since_last': None, 'needs_refresh': True}
    ...
    def get_award_books(
        ...
        page: int = 1,
        limit: int = 20,
        include_displayable_only: bool = False,
        ...
        try:
            ...
            if include_displayable_only:
                query = query.filter_by(is_displayable=True)
            if keyword:
    ...
    def get_distinct_years(self, award_id: int | None = None) -> list[int]:
        """获取不重复的年份列表（可按奖项过滤）"""
        try:
            query = db.session.query(AwardBook.year).distinct()
            if award_id:
                query = query.filter_by(award_id=award_id)
