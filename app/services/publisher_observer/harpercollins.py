...
_ATOM_SOURCE_URL = (
    "https://www.harpercollins.com/blogs/literary-hub/new-releases.atom"
)
    ...
    def from_path(cls, path: str | Path) -> FixedTemplateFallback:
        ...
        return cls(
            _candidate=TemplateCandidate(
                ...
                reason=reason,
                verified=(
                    verified_value if isinstance(verified_value, bool) else True
                ),
            ),
...
def _validate_manifest_metadata(fixtures: list[dict[str, Any]]) -> None:
    for item in fixtures:
        ...
        if name.endswith(".atom"):
            ...
            name.endswith(_COLLECTION_SUFFIXES)
            or name == "fixed-ai-candidate.json"
        ) and _normalize_collection_url(source_url) != source_url:
            raise ValueError(
                "collection or AI source_url must be the canonical HarperCollins collection URL"
            )
        if name.endswith(_COLLECTION_SUFFIXES) and "page" in item:
            ...
            if (
                ...
                raise ValueError(
                    "collection page metadata must match the source_url page"
                )
        if name.endswith(_COLLECTION_SUFFIXES) and "next_url" in item:
            ...
            if next_url is not None and (
                ...
                or _normalize_collection_url(next_url) != next_url
            ):
                raise ValueError(
                    "collection next_url must be a canonical HarperCollins collection URL"
                )
...
def observe_fixture_manifest(
    ...
    for item in fixtures:
        ...
        if name.endswith(".atom"):
            ...
            if record is not None and source_url != record.source_url:
                raise ValueError(
                    "product source_url must equal the fixture canonical product URL"
                )
            evidence.append(
                replace(
                    item_evidence,
    ...
    collection_evidence, collection_candidates = _observe_collections(
        base_dir, collection_items, manifest_digest
    )
...
def _observe_collections(
    ...
    for item in items:
        ...
        if key not in pairs:
            ...
            order.append(key)
        extraction_method = (
            ExtractionMethod.CSS if method == "css" else ExtractionMethod.XPATH
        )
    ...
    for key in order:
        ...
        if css is not None and xpath is not None and any(
            css.get(field) != xpath.get(field)
            for field in ("source_url", "next_url", "page")
        ):
    ...
    for key in order:
        if key not in conflicted_keys:
            continue
        error_code = (
            "DUPLICATE_COLLECTION_MEMBER"
            if key in duplicate_member_keys
            else "DUPLICATE_SOURCE_URL"
        )
        evidence.extend(
            _collection_conflict_evidence(base_dir, item, method, error_code)
            for method, item in members[key]
        )

    traversal_order = [key for key in order if key not in conflicted_keys]

    source_to_key = {
        _pair_source_url(pairs[pair_key]): pair_key for pair_key in traversal_order
    }
    referenced_urls = {
        next_url
    ...
    roots = [
        pair_key
        for pair_key in traversal_order
        if _pair_source_url(pairs[pair_key]) not in referenced_urls
    ]
    visited: set[str] = set()
    ...
    for root in roots + [key for key in traversal_order if key not in roots]:
        ...
        while current_key is not None and current_key not in suppressed:
            if current_key in visited:
                evidence.append(
                    _cycle_evidence(current_key, pairs[current_key], manifest_digest)
                )
                break
            ...
            if page_count >= MAX_COLLECTION_PAGES:
                ...
                _suppress_collection_chain(
                    current_key, pairs, source_to_key, visited, suppressed
                )
                break

            visited.add(current_key)
            page_count += 1
            css_urls, css_evidence = _read_collection_output(
                base_dir, pair.get("css"), ExtractionMethod.CSS
            )
            xpath_urls, xpath_evidence = _read_collection_output(
                base_dir, pair.get("xpath"), ExtractionMethod.XPATH
            )
            stop_series = False
            ...
            if EvidenceStatus.ACCESS_BLOCKED in {
                ...
                evidence.append(
                    _evidence(
                        ...
                        0,
                        _combined_digest(
                            css_evidence.input_sha256, xpath_evidence.input_sha256
                        ),
                        "TEMPLATE_DRIFT",
                    )
                )
            ...
            if stop_series:
                ...
                _suppress_collection_chain(
                    next_key, pairs, source_to_key, visited, suppressed
                )
                break
            next_url = _pair_next_url(pair)
            next_key = source_to_key.get(next_url) if next_url is not None else None
            ...
            if next_key is not None and next_key in visited:
                evidence.append(
                    _cycle_evidence(current_key, pair, manifest_digest)
                )
                break
...
def _suppress_collection_chain(
    ...
    while (
        ...
        and current_key not in suppressed
    ):
...
def _parse_editions(value: object) -> tuple[EditionObservation, ...]:
    ...
    for index, variant in enumerate(value):
        ...
        isbn = next(
            (
                ...
                if is_valid_isbn13(candidate := variant.get(field))
            ),
            None,
        )
    ...
    return tuple(
        EditionObservation(book_format, isbn, isbn == main_isbn)
        for _, _, book_format, isbn, _ in observed
    )
...
def _normalize_product_url(value: object) -> str | None:
    ...
    if (
        ...
        or not parsed.path.startswith(_PRODUCT_PREFIX)
    ):
        return None
...
def _normalize_collection_url(value: object) -> str | None:
    ...
    if (
        parsed.scheme != "https"
        or parsed.netloc != _ALLOWED_HOST
        or parsed.path != _COLLECTION_PATH
        or parsed.fragment
    ):
        return None
