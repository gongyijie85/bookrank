...
class TestUserPreference:
    ...
    def test_create_user_preference(self, db):
        """测试创建用户偏好"""
        preference = UserPreference(session_id='test_session_123', view_mode='grid')
    ...
    def test_user_preference_to_dict(self, db):
        """测试转换为字典"""
        preference = UserPreference(session_id='test_session_789', view_mode='list')
        db.session.add(preference)
        db.session.commit()
...
class TestUserCategory:
    ...
    def test_create_user_category(self, db):
        ...
        category = UserCategory(session_id='test_session_cat', category_id='hardcover-fiction')
        db.session.add(category)
        db.session.commit()

        result = UserCategory.query.filter_by(session_id='test_session_cat', category_id='hardcover-fiction').first()
...
class TestUserViewedBook:
    ...
    def test_create_viewed_book(self, db):
        ...
        viewed = UserViewedBook(session_id='test_session_view', isbn='9780143127550')
        db.session.add(viewed)
        db.session.commit()

        result = UserViewedBook.query.filter_by(session_id='test_session_view', isbn='9780143127550').first()
...
class TestBookMetadata:
    ...
    def test_book_metadata_to_dict(self, db):
        """测试转换为字典"""
        metadata = BookMetadata(isbn='9780143127551', title='Another Book', author='Another Author')
        db.session.add(metadata)
        db.session.commit()
...
class TestSearchHistory:
    ...
    def test_create_search_history(self, db):
        """测试创建搜索历史"""
        history = SearchHistory(session_id='test_session_search', keyword='Python', result_count=10)
...
class TestAward:
    ...
    def test_award_to_dict(self, db):
        """测试转换为字典"""
        award = Award(name='测试奖项', name_en='Test Award', country='中国')
        db.session.add(award)
        db.session.commit()
...
class TestAwardBook:
    ...
    def test_create_award_book(self, db):
        """测试创建获奖图书"""
        # 先创建奖项
        award = Award(name='测试奖', name_en='Test Award', country='US')
        db.session.add(award)
        db.session.commit()
...
class TestSystemConfig:
    ...
    def test_create_system_config(self, db):
        """测试创建系统配置"""
        config = SystemConfig(key='site_name', value='BookRank', description='网站名称')
    ...
    def test_get_value(self, db):
        """测试获取配置值"""
        config = SystemConfig(key='max_results', value='50')
        db.session.add(config)
        db.session.commit()
...
class TestBook:
    ...
    def test_book_price_handling_zero(self):
        ...
        book = Book.from_api_response(book_data, 'fiction', '小说', 'List', '2024-01-01', {})
        assert book.price == '未知'


# ==================== NewBook 刚上市徽章判定 ====================
# （由 tests/test_new_book_service.py 拆分迁移而来）
