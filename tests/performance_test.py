...
def test_database_performance():
    ...
    with app.app_context():
        ...
        # 测试查询
        start_time = time.time()
        WeeklyReport.query.all()
        end_time = time.time()
        print(f'查询所有周报数据: {end_time - start_time:.4f}秒')

        # 测试按日期查询
        start_time = time.time()
        WeeklyReport.query.order_by(WeeklyReport.report_date.desc()).limit(10).all()
        end_time = time.time()
...
def test_export_performance():
    ...
    with app.app_context():
        ...
        # 测试PDF导出
        start_time = time.time()
        export_service.export_weekly_report_pdf(report)
        end_time = time.time()
        print(f'PDF导出时间: {end_time - start_time:.4f}秒')

        # 测试Excel导出
        start_time = time.time()
        export_service.export_weekly_report_excel(report)
        end_time = time.time()
...
def test_report_generation_performance():
    ...
    with app.app_context():
        ...
        # 生成周报（使用默认摘要，不调用AI）
        report_service.generate_report(week_start, week_end)
        end_time = time.time()
