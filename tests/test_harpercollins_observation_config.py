...
FIXTURE_DIR = (
    Path(__file__).parent / "fixtures" / "publisher_observer" / "harpercollins"
)
...
def _write_fixed_candidate(
    directory: Path, *, selector: str = ".product-card a", verified: bool = False
) -> Path:
...
def test_untrusted_manifest_urls_are_rejected_before_document_reads(
    ...
    with pytest.raises(ValueError, match=r"source_url|next_url"):
        api.observe_fixture_manifest(
            _write_manifest(tmp_path / "manifest.json", [item])
        )
...
def test_collection_pair_metadata_must_agree(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(ValueError, match="metadata"):
        api.observe_fixture_manifest(
            _write_manifest(tmp_path / "manifest.json", [css, xpath])
        )
    ...
    _write_json(
        product,
        {
            "product": {
                ...
                "images": [
                    {"alt": "Duplicate Format by A Writer (9780063416178)"}
                ],
                "variants": [
...
def test_duplicate_collection_strategy_member_is_explicit_and_digest_complete(
    ...
    report = api.observe_fixture_manifest(
        _write_manifest(tmp_path / "manifest.json", fixtures)
    )

    duplicates = [
        item for item in report.evidence if item.error_code == "DUPLICATE_COLLECTION_MEMBER"
    ]
    assert duplicates
...
def test_duplicate_collection_source_across_pages_fails_all_conflicting_inputs(
    ...
    conflicts = [
        item for item in report.evidence if item.error_code == "DUPLICATE_SOURCE_URL"
    ]
...
def test_collection_rows_rejected_by_both_strategies_remain_explicit_drift(
    ...
    rejected = [
        item for item in report.evidence if item.error_code == "COLLECTION_ROWS_REJECTED"
    ]
...
def test_collection_pagination_stops_explicitly(
    ...
    for page in range(1, page_count + 1):
        ...
        next_url = (
            f"https://www.harpercollins.com/collections/new-releases?page={page + 1}"
            if page < page_count
            else None
        )
        for method in ("css", "xpath"):
    ...
    if mode == "limit":
        assert not any(
            marker in item.document_id
            for marker in ("page-11", "page-12")
...
def test_collection_root_entering_cycle_emits_duplicate_page_evidence(
    ...
    api = _observer_api()
    urls = [
        f"https://www.harpercollins.com/collections/new-releases?page={page}"
        for page in (1, 2, 3)
    ]
    fixtures: list[dict[str, object]] = []
    for index, (source_url, next_url) in enumerate(
        zip(urls, (urls[1], urls[2], urls[1]), strict=True), start=1
    ):
        fixtures.extend(
            _write_collection_pair(
                tmp_path,
...
def test_multiple_drift_pages_consume_one_fallback_call(tmp_path: Path) -> None:
    ...
    report = api.observe_fixture_manifest(
        _write_manifest(tmp_path / "manifest.json", fixtures), fallback=fallback
    )
...
def test_fallback_fixture_mismatch_fails_closed(tmp_path: Path) -> None:
    ...
    other_dir.mkdir()
    fallback = api.FixedTemplateFallback.from_path(
        _write_fixed_candidate(other_dir, selector=".caller-owned")
    )
...
def test_non_harper_source_is_rejected_before_documents_are_read(
...
def test_non_object_manifest_entries_are_rejected_before_fixture_reads(
