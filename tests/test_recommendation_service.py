...
class TestExtractKeywords:
    ...
    def test_none_text(self, rec_service):
        assert rec_service._extract_keywords(None) == []
...
class TestGenerateRecommendationReason:
    ...
    def test_no_interests(self, rec_service):
...
class TestGetSmartRecommendations:
    ...
    def test_with_session(self, app, db, rec_service, sample_books):
        with app.app_context(), patch.object(rec_service, 'get_personalized_recommendations') as mock_p:
            mock_p.return_value = {'recommendations': [{'id': 1}], 'reason': 'test', 'based_on': 'personalized'}
            result = rec_service.get_smart_recommendations(session_id='test')
            assert result['strategy'] == 'hybrid'

    def test_without_session(self, app, db, rec_service, sample_books):
        with app.app_context():
            result = rec_service.get_smart_recommendations()
            assert 'strategy' in result

    def test_limit_clamping(self, rec_service):
