...
**核心功能**：自动检测英国亚马逊是否有货，如无货则切换到美国亚马逊
...
输入ISBN-10
    ...
    ▼
┌─────────────────┐
│ 生成UK链接       │ https://www.amazon.co.uk/dp/{ISBN-10}
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ 检查UK站是否有货 │ ◄─── Scrape.do API
│                  │     检查页面是否有 "In stock" / "Out of stock"
└────────┬────────┘
         │
    ┌────┴────┐
    │         │
...
|------|------|
| 亚马逊链接(UK) | 英国亚马逊商品页链接 |
| 亚马逊链接(US) | 美国亚马逊商品页链接（如UK无货） |
| UK有货状态 | In Stock / Out of Stock / Unknown |
| US有货状态 | In Stock / Out of Stock / Unknown |
| 最终链接 | 实际有货的链接（优先UK） |

**链接格式**：
- UK: `https://www.amazon.co.uk/dp/{ISBN-10}`
- US: `https://www.amazon.com/dp/{ISBN-10}`

**防封策略**：
- Scrape.do API 自动处理代理轮换
- 随机延迟 1-3 秒
- 缓存检查结果（24小时有效）
...
| **P** | **亚马逊链接(UK)** | 生成得到 | 英国亚马逊商品页 |
| **Q** | **亚马逊链接(US)** | 生成得到 | 美国亚马逊商品页 |
| **R** | **UK有货状态** | 爬取判断 | In Stock / Out of Stock / Unknown |
| **S** | **US有货状态** | 爬取判断 | In Stock / Out of Stock / Unknown |
| **T** | **最终使用链接** | 智能选择 | 优先UK，UK无货则用US |
| **U** | **书名(英)** | 亚马逊获取 | 英文原名 |
| **V** | **作者(英)** | 亚马逊获取 | 英文作者名 |
| **W** | **出版社(英)** | 亚马逊获取 | 英文出版社 |
| **X** | **图书简介(英)** | 亚马逊获取 | 英文原文简介 |
| **Y** | **书名(中)** | AI翻译 | 中文翻译 |
| **Z** | **作者(中)** | AI翻译 | 中文翻译 |
| **AA** | **出版社(中)** | AI翻译 | 中文翻译 |
| **AB** | **图书简介(中)** | AI翻译 | 中文翻译 |
| **AC** | **小红书文案** | AI生成 | 种草笔记文案 |
| **AD** | **封面图片URL** | 亚马逊获取 | 可用于配图 |

> **列数变化**：从12列 → 30列（新增18列）
> - 亚马逊链接相关：1列 → 5列（UK链接、US链接、UK状态、US状态、最终链接）
...
| 亚马逊爬取 | Scrape.do API | 获取商品信息+库存状态 |
| 库存检查 | Scrape.do API | 检查UK/US亚马逊库存 |
| 翻译 | 智谱AI | 已有，集成 |
| 小红书文案 | 智谱AI | 已有，集成 |
...
**关键设计点**：
- 缓存策略：亚马逊数据（30天）、翻译结果（永久）、文案（7天）、库存状态（24小时）
- 失败处理：某数据源失败自动切换备选，全部失败记录但继续处理下一本
- 并发控制：支持10本并行处理（可配置），避免API限流
- 库存检查：UK无货自动切换US，US也无货则标记失败
...
| 1.5 | UK/US库存检查 | 0.5天 | `inventory_checker.py` |
| 1.6 | 核心处理逻辑 | 0.5天 | `xiaohongshu_service.py` |
| **Phase 2** | 翻译+文案 | 1天 | |
| 2.1 | 多字段翻译 | 0.25天 | 扩展翻译服务 |
| 2.2 | 小红书文案生成 | 0.5天 | 文案生成Prompt |
| 2.3 | 进度显示 | 0.25天 | WebSocket/轮询 |
| **Phase 3** | 导出+历史 | 0.5天 | |
| 3.1 | 18列Excel导出 | 0.25天 | 导出功能 |
...
| 里程碑 | 目标 | 验收标准 |
|--------|------|----------|
| M1 | Day 3 | 能上传Excel并智能识别ISBN，成功获取亚马逊信息+UK库存 |
| M2 | Day 4 | UK无货自动切换US，生成完整Excel（含库存状态） |
| M3 | Day 5.5 | 完整功能可用，UI完善，并发处理正常 |
...
| Scrape.do | ¥200/月 | 约$28/月，20000次请求（获取信息+UK/US库存检查） |
| Google Books | 免费 | 无限制 |
| 服务器资源 | 现有 | 无额外成本 |
| **合计** | **¥200/月** | 主要是Scrape.do费用 |

> **成本变化**：¥100/月 → ¥200/月（新增US站点库存检查）
> - 每本书约需2次Scrape.do请求（商品信息 + 库存检查）
> - 预估处理量：500-1000本/月
...
| UK亚马逊无货 | 中 | 自动切换US站点，保留两条链接供用户选择 |
...
| 库存检查成功率 | ≥95% | 成功获取UK/US库存状态数/总数 |
| 小红书文案可用率 | ≥80% | 成功生成文案数/总数 |
| 平均处理时间 | <5秒/本 | 含UK/US库存检查（并发10本） |
| 用户满意度 | >4.5/5 | 反馈调查 |
| 日处理量 | >500本 | 系统统计 |
